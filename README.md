...
| :--- | :--- | :--- | :--- |
| 🚀 **SmartDNS + VLESS** | **Только DNS (`192.168.1.118`)** | Прямой (0 мс задержки) | 🟢 Да (Встроен VLESS) |
| ⚡ **Zapret DPI Bypass** | Шлюз + DNS (`192.168.1.118`) | Нативный (0 мс задержки) | ❌ Нет |
| 🛡️ **Гибридный** | Шлюз + DNS (`192.168.1.118`) | Нативный (0 мс задержки) | 🟢 Да |
...
👉 **`http://<IP-Raspberry-Pi>:8080`** *(например, `http://192.168.1.118:8080`)*
...
3. Установите:
   ...
   - **Основной DNS (Primary DNS)**: **`192.168.1.118`** *(IP вашей Raspberry Pi)*
...
2. Установите **IP-адрес ➔ Вручную**:
   - **IP-адрес**: `192.168.1.50` *(свободный IP)*
   - **Маска подсети**: `255.255.255.0`
   - **Основной шлюз (Gateway)**: **`192.168.1.118`** *(IP вашей Raspberry Pi)*
   - **Основной DNS**: **`192.168.1.118`**
